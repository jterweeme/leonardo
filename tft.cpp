...
uint8_t TFT::read8() const
{
    ...
    dst = (PINB & 3<<4) >> 4 | (PIND & 1<<1) << 1 | (PIND & 1) << 3 | (PIND & 1<<4) |
        (PINC & 1<<6) >> 1 | (PIND & 1<<7) >> 1 | (PINE & 1<<6) << 1;
...
inline void TFT::write8(uint8_t x) const
{
    ...
    PORTB |= (x & 3<<0) << 4;   // data 0 & 1 -> PB4,PB5
    PORTC |= (x & 1<<5) << 1;   // data 5     -> PC6
    PORTD |= (x & 1<<2) >> 1;   // data 2     -> PD1
    PORTD |= (x & 1<<3) >> 3;   // data 3     -> PD0
    PORTD |= (x & 1<<4);        // data 4     -> PD4
    PORTD |= (x & 1<<6) << 1;   // data 6     -> PD7
    PORTE |= (x & 1<<7) >> 1;   // data 7     -> PE6
